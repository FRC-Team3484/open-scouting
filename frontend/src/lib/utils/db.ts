...
export class OpenScoutingDB extends Dexie {
  match_scouting! : Dexie.Table<{uuid: string, data: any, user_uuid: string, year: number, event_code: string, event_name: string, event_type: string, event_city: string, event_country: string, event_start_date: string, event_end_date: string, synced: boolean}, string>;
  ...
  constructor() {
    ...
    this.version(1).stores({
      match_scouting: "&uuid, data, user_uuid, year, event_code, event_name, event_type, event_city, event_country, event_start_date, event_end_date, synced",
